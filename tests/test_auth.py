...
class AuthenticationTests(unittest.TestCase):
    ...
    def test_dashboard_activity_requires_authentication(self) -> None:
        for path in ["/dashboard/activity", "/dashboard/activity.md", "/dashboard/activity.csv"]:
            with self.subTest(path=path):
    ...
    def test_dashboard_proposal_payment_requires_authentication(self) -> None:
        response = self.client.post("/dashboard/proposals/example-proposal-id/payment", data={"action": "mark_paid"}, follow_redirects=False)
        self.assertIn(response.status_code, {303, 401})
        if response.status_code == 303:
            self.assertTrue(response.headers["location"].startswith("/login"))

    def test_dashboard_proposal_payment_send_requires_authentication(self) -> None:
        response = self.client.post("/dashboard/proposals/example-proposal-id/payment/send", data={"action": "request"}, follow_redirects=False)
        self.assertIn(response.status_code, {303, 401})
        if response.status_code == 303:
            self.assertTrue(response.headers["location"].startswith("/login"))
