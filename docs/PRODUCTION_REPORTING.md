...
```bash
vendorverdict --demo --no-live-evidence --save-report --db ./data/vendorverdict.sqlite3
```

List recent reports:

```bash
vendorverdict --list-reports --db ./data/vendorverdict.sqlite3
```

Show one report:

```bash
vendorverdict --show-report REPORT_ID --db ./data/vendorverdict.sqlite3
```

Export one report to Markdown:

```bash
vendorverdict --export-markdown REPORT_ID ./exports/report.md --db ./data/vendorverdict.sqlite3
```
